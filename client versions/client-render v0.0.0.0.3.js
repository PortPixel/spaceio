...
/*	deltaX = mousex(event) - myGamePiece.x;
	...
	if (!spectate) {
		...
		users[myId].clientData = clientData;
	}
			...
			for (i2 = 0; i2 < users[playerIds[i]].cells.length; i2++) {
				...
				for (i3 =0; i3 < (collength - 1); i3+=6) {
					...
					if (getDistance(cellx - collects[i3] , celly - collects[i3 + 1]) < (cellsize/2 + collects[i3 + 2])*2)  {
						...
						cellsize = Math.sqrt((Math.pow(cellsize, 2)) + Math.pow(collects[i3 + 2] , 2)/3);
						if (i2 != 0) {
	...
	function roundRectTwoCorners(ctx, x, y, width, height, radius, fill, stroke) {
		if (typeof stroke == "undefined" ) {
			stroke = true;
		}
		if (typeof radius === "undefined") {
	...
	function renderUpgradeBar (x,y,text,shortcut) {
		ctx.fillStyle = "rgba(150, 150, 150, .5)";
		roundRect(ctx, x, y, 160, 18, 10, true,false);
		ctx.fillStyle = "rgb(200, 50, 50)";
		roundRectTwoCorners(ctx, x+130, y+2, 28, 14, 8, true,false);
		ctx.fillStyle = "black";
		...
		ctx.fillText(shortcut,x+110,y+13);
	}
	
	if (mouseposx > 140 && mouseposx < 168) {
		if (mouseposy > 400 && mouseposy < 561 ) {
			var upgradeHoveredOver = Math.ceil((mouseposy-400)/21)
			console.log(upgradeHoveredOver)
		}
	}
	renderUpgradeBar(10,400,'Health Regen','(1)');
	renderUpgradeBar(10,421,'Max Health','(2)');
	renderUpgradeBar(10,442,'Body Damage','(3)');
	renderUpgradeBar(10,463,'Laser Speed','(4)');
	renderUpgradeBar(10,484,'Laser Penetration','(5)');
	renderUpgradeBar(10,505,'Laser Damage','(6)');
	renderUpgradeBar(10,526,'Reload','(7)');
	renderUpgradeBar(10,547,'Movement Speed','(8)');

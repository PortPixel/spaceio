...
socket.on('update', function (data) {
	...
	//if (updated == false) {users = data.users; usersFromServer = data.users; updated = true;} else {usersFromServer = data.users}
	//usersFromServer = data.users
	users = data.users;
	lasers = data.lasers;
	/*physicsRepeats += Math.round(Date.now() - data.time * (1000/60))*/
...
/*	deltaX = mousex(event) - myGamePiece.x;
	...
	if (dev) {
		ctx.beginPath();
		ctx.arc((camerax - usersFromServer[myId].cells[0].x) * zoom, (cameray - usersFromServer[myId].cells[0].y) * zoom, usersFromServer[myId].cells[0].size  * zoom, 0 , 2*Math.PI);
		ctx.fillStyle = 'white';
		ctx.fill();
		ctx.closePath();
	}

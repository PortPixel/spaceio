...
/*	deltaX = mousex(event) - myGamePiece.x;
	...
	function renderCells () {
		...
		for (i = 0; i < playerIds.length; i++) {
			...
			for (i2 = 0; i2 < users[playerIds[i]].cells.length; i2++) {
				...
				var celldir = users[playerIds[i]].cells[i2].gunDir;
	...
	function cellPhysics () {
		...
		for (i = 0; i < playerIds.length; i++) {
			...
			for (i2 = 0; i2 < users[playerIds[i]].cells.length; i2++) {
				...
				createParticle(cellx,celly,Math.sin(toRadians(celldir)) * cellv ,Math.cos(toRadians(celldir)) * cellv,Math.random() * 100 /*,Math.round(15 + Math.random() * 30)*/);
	...
	function createParticle(x,y,xv,yv,size,number) {
		if (true/*number == 'undefined'*/) {
			particles.push(x,y,xv*(0.9+Math.random()*0.1),yv*(0.9+Math.random()*0.1),size,'rgba(' + (225 + Math.random() * 30) + ','+ (120 + Math.random() * 100) + ', 255,0.5)');
		} else {
			var i;
			for (i=0; i<number; i++) {
				particles.push(x,y,xv*(0.9+Math.random()*0.1),yv*(0.9+Math.random()*0.1),size,'rgba(' + (225 + Math.random() * 30) + ','+ (120 + Math.random() * 100) + ', 255,0.5)');
	...
	function renderUpgradeBar (x,y,text,number,upgrades) {
	...
	if (mouseposx > 140 && mouseposx < 168) {
		if (mouseposy > 400 && mouseposy < 561 ) {
	...
	renderUpgradeBar(10, 400, 'Health Regen', 1, users[myId].upgrades.regen);
	renderUpgradeBar(10, 421, 'Max Health', 2, users[myId].upgrades.maxHealth);
	renderUpgradeBar(10, 442, 'Body Damage', 3, users[myId].upgrades.bodyDam);
	renderUpgradeBar(10, 463, 'Laser Speed', 4, users[myId].upgrades.laserSpeed);
	renderUpgradeBar(10, 484, 'Laser Penetration', 5, users[myId].upgrades.laserPen);
	renderUpgradeBar(10, 505, 'Laser Damage', 6, users[myId].upgrades.laserDam);
	renderUpgradeBar(10, 526, 'Reload', 7, users[myId].upgrades.reload);
	renderUpgradeBar(10, 547, 'Movement Speed', 8, users[myId].upgrades.moveSpeed);
